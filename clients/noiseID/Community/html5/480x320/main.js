...
function animate(){
    ...
    tl.addLabel('start')
      ...
      .to('#logo', 0, { scale: 0.6, x: -146, y: -93, ease: Power1.easeInOut })
      //  Adjust logo colours so as not to lose logo against background
      .to(iamsLogoPaths, 0, { fill: '#F2F2F2' })
      .to('#bg', 1, { autoAlpha: 1 })
      .to('#logo', 0.5, { autoAlpha: 1 }, "-=0.5")
      .to(['#bg'], 0.75, { scale: 1.5, x: "-=100", y: "-=50", ease: Power1.easeInOut }, '-=0.75')
      //  Target looking for dog nose
      .to('#target', 0.75, { scale: 1, ease: Power4.easeOut }, "-=0.5")
      ...
      .to(['#bg', '#target'], 0.75, { x: "+=80", ease: Power1.easeInOut }, '-=0.75')
